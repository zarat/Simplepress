...
function add_field( $item_id, $field_key, $field_value ) {
    $system = new system();
    $config['insert'] = "object_meta (`meta_item_id`, `meta_key`, `meta_value`)";
...
function delete_field( $field_id ) {
    $system = new system();
    $item_id = $system->select( array( 'select' => '*', 'from' => 'object_meta', 'where' => 'meta_id=' . $field_id ) );
    $system->delete( array( 'from' => 'object_meta', 'where' => 'meta_id=' . $field_id ) );
...
if( isset( $_POST['item_id'] ) ) {
    ...
    $system = new system();
    $config = array( "insert" => "object_meta (`meta_item_id`, `meta_key`, `meta_value`)", "values" => "($item_id, '$customfield_key', '$customfield_value')");
